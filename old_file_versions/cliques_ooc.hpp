...
inline  bool clique_prefilter(const Embedding<VertexId >* embedding,const VertexId cand )  {
    return degree[cand] >=K - 1;
}

inline  bool clique_filter( const Embedding<uint32_t>* embedding)  {
...
inline bool expand(const uint32_t step)  {
 ...
 static void activate_nodes(){
  for (uint32_t i = 0; i < NB_NODES; i++) {
    if (clique_prefilter(NULL,i))//should_be_active(i))
      for (size_t idx = 0; idx < degree[i]; idx++) {
        if (clique_prefilter(NULL,edges_full[adj_offsets[i] + idx].dst) && edges_full[adj_offsets[i] + idx].dst >
                                                               i)
          active[no_active++] = adj_offsets[i] + idx;
      }
  }
