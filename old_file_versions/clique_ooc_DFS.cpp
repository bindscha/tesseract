...
void explore(uint32_t src, clique_vector set, uint32_t step, int tid){
  int count = 0;
  per_thread_data[tid]++;
  bool is_cand = canonic_check(src,set,step);
  if(!is_cand) return;
//  is_cand = exec_filter<uint32_t,clique_vector>(&filter, src, set, step);
//  if(!is_cand) return;
  ...
  if(step < K - 1) {
    ...
    FOREACH_EDGE_FWD(src, dst)
      if (!should_be_active(dst)) continue;
  ...
  while(curr_item    < no_active) {
    ...
    if(thread_work[tid].start == thread_work[tid].stop) goto end;
        ...
        for (; thread_work[tid].start < thread_work[tid].stop; thread_work[tid].start++) {
          ...
          if(!should_be_active(src)) continue;
          uint32_t dst;
          FOREACH_EDGE_FWD(src, dst)
                  if(should_be_active(dst))
